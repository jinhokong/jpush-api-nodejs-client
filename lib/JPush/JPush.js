...
function JPushClient(
  ...
  if (!appKey || !masterSecret) {
    throw JError.InvalidArgumentError(
      "appKey and masterSecret are both required."
    );
  }
...
function getScheduleList(page, callback) {
  if (typeof page !== "number") {
    throw new JError.InvalidArgumentError(
      "Invalid argument, it can only be set to the Number."
    );
  }
...
function getSchedule(scheduleId, callback) {
  if (typeof scheduleId !== "string") {
    throw new JError.InvalidArgumentError(
      "Invalid argument, it can only be set to the String."
    );
  }
  var url = SCHEDULE_API_URL + "/" + scheduleId;
  return _request(this, url, null, "GET", callback);
  ...
  var _callback = function(err, res, body) {
    if (err) {
      ...
      if (times < client.maxTryTimes) {
        ...
        return callback(
          new JError.APIConnectionError("Connect timeout. Please retry later.")
        );
      }
    }
